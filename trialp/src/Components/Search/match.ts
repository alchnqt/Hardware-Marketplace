...
const wordCharacterRegex = /[a-z0-9_]/i // /[a-z0-9_]/i;

const whitespacesRegex = /\s+/;// /\s+/;
...
export default function match(text, query, options) {
  ...
  const cleanedTextArray = Array.from(text).map((x) => (x));
